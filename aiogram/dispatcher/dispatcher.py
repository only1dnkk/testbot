...
class Dispatcher(Router):
        ...
        while True:
            try:
                ...
                loggers.dispatcher.warning(
                    "Заснули на %f секунд(tryings = %d, bot id = %d)",
                    backoff.next_delay,
                    backoff.counter,
                    bot.id,
                )
    ...
    async def _polling(
        ...
        **kwargs: Any,
        ...
        loggers.dispatcher.info(
            "\033[1;31mПоллинг был запущен в боте @%s - %r \033[0m", user.username, user.full_name
        )
        try:
    ...
    async def start_polling(
        ...
        **kwargs: Any,
        ...
        async with self._running_lock:  # Prevent to run this method twice at a once
            ...
            loggers.dispatcher.info("\033[1;35mПоллинг запущен\033[0m")
            try:
